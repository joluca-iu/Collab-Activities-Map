...
def get_community_partners_by_program(portal_id, portal_name, program_id, program_name):
    #Navigate to ActivitesMap Dir
    #Create a folder path for each portal
    out_dir = DATA_DIR / "raw" / portal_name
    #Create a file path for each program
    ...
    payload = json.dumps({
    "query": "mutation GetCommunityOrgFullFunc($input: GetCommunityOrgFullFuncInput!) { getCommunityOrgFullFunc(input: $input) { results {  id name street street2 zipcode city state county country latitude longitude type description url phone email archived status activityCnt activityName role contactNames contactEmails sectionCnt courses unitCnt unitNames externalId } } }",
    "variables": {
        "input": {
        "pPortalId": portal_id,
        "pToken": TOKEN,
    ...
    ##Double checking that valid API data collected
    if json_data.get("erros"):
    ...
    for portal in portals:
        ...
        for program in programs:
            ...
            if program_id and program_id != "None":
                df = pd.DataFrame()
                df = get_community_partners_by_program(portal_id, portal_name, program_id, program_name)
