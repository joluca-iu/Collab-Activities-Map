...
function apply_markers(schools_geojson) {
  ...
  schools_geojson.features.forEach(feature => {
    ...
    const tabButtons = entities.map((entity, i) => {
      const label = entity?.portal_name ?? entity?.name ?? `Entity ${i + 1}`;
      return `<button class="popup-tab-btn" data-index="${i}" onclick="switchPopupTab('${popupId}', ${i}); return false;" data-index="${i}">${label}</button>`;
    }).join(' | ');
    ...
    const panels = entities.map((entity, i) => {
      ...
      return `
        <div class="popup-entity" data-index="${i}" style="display: ${i === 0 ? 'block' : 'none'};">
          ...
          <div class="section-label">Campuses</div>
